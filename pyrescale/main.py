...
class Job:
    ...
    def get_odbfile(self, rescale_jobid = None):
        self.rescale_odblist = Job.find_file(file=".odb", rescale_jobid=rescale_jobid)

        if len(self.rescale_odblist) > 1:
            raise Exception("More than one odb file found")
        else: 
            self.rescale_odblist = self.rescale_odblist[0]
            self.storage_id.append({"id": self.rescale_odblist[2]})

    def create(self, name, mainfile, abq_version):
        if not mainfile:
            raise ValueError("Enter main file name")
              ...
              'jobanalyses': [
                  {
                    ...
                    #   'command': f'abaqus job={mainfile} cpus=$RESCALE_CORES_PER_SLOT mp_mode=mpi double=both interactive',
                      'command': f'abaqus job={mainfile} interactive\nabaqus view noGUI={mainfile}',
                      'hardware': {
    ...
    def find_file(file, rescale_jobid, max_size=1):
        ...
        #     raise Exception(f"More than {max_size} {file} file found")    
            ...
            return [[rescale_jobid, file["name"], file["id"]] for file in json.loads(resp.content)['results']]
